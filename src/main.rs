...
fn main() {
    ...
    let mut wayland_state = wayland::State {  };
    let mut server = wayland_server::Display::<wayland::State>::new().unwrap();

    server.handle().create_global::<wayland::State, WlCompositor, _>(4, ());
    server.handle().create_global::<wayland::State, WlShm, _>(2, ());
    server.handle().create_global::<wayland::State, XdgWmBase, _>(2, ());
